...
import AdminUserLoanList from "./pages/AdminUserLoanList";
...
const globalStyles = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .bg-overlay {
    position: absolute;
    inset: 0;
    background: linear-gradient(135deg, #0f172a08, #6366f10f 50%, #ffffff1a);
    z-index: 1;
  }

  /* Keep all auth screens visually identical in background transparency. */
  .page.auth-page .bg-overlay {
    background: linear-gradient(135deg, #0f172a08, #6366f10f 50%, #ffffff1a);
  }

  /* Shared background opacity for loan-application pages. */
  .page-bg-image.loan-app-bg,
  .page-bg-image.loan-details-bg,
  .page-bg-image.user-loan-details-bg {
    background: linear-gradient(135deg, rgba(15, 23, 42, 0.07), rgba(99, 102, 241, 0.11) 50%, rgba(255, 255, 255, 0.24)) !important;
  }

  .card-wrap {
    position: relative;
    z-index: 10;
    margin-right: clamp(28px, 7vw, 130px);
    display: flex;
  ...
  .index-card {
    ...
    box-shadow: 0 18px 44px rgba(15,23,42,0.14), 0 1px 0 rgba(255,255,255,0.76) inset;
    display: flex;
    flex-direction: column;
    align-items: center;
    gap: 22px;
  ...
  .index-logo-panel {
    width: 100%;
    border-radius: 14px;
    background: linear-gradient(180deg, rgba(255,255,255,0.74) 0%, rgba(238,243,252,0.62) 100%);
  ...
  .index-btn-user {
    ...
    border: none;
    border-radius: 13px;
    background: linear-gradient(135deg, #5f63f2, #3f3fce);
    color: #fff;
  ...
  .index-btn-admin {
    ...
    border: none;
    border-radius: 13px;
    background: linear-gradient(135deg, #ff7a13, #f76300);
    color: #fff;
    font-family: 'Inter', sans-serif;
    font-size: 15px;
    font-weight: 800;
...
export function PageBg({ children, pageClass = "" }) {
  const pageClassName = `page ${pageClass}`.trim();
  return (
...
function AppContent() {
  ...
  const [draftLoanId, setDraftLoanId] = useState(null);

  useEffect(() => {
    // Keep CSS var in sync with the hashed asset path at runtime.
    document.documentElement.style.setProperty("--page-bg-url", `url(${bgImage})`);
  }, []);
  ...
  const navigate = (nextPage, params) => {
    ...
    if (params?.draftLoanId != null) setDraftLoanId(params.draftLoanId);
    if (nextPage !== "admin-loan-detail") setAdminLoanId(null);
    if (nextPage !== "user-loan-detail") setUserLoanId(null);
    if (nextPage !== "apply-loan") setDraftLoanId(null);
  };
    ...
    switch (page) {
      ...
      case "user-login":
        return (
          <UserLogin
            navigate={setPage}
            onLoginSuccess={() => setPage("documentation")}
          />
        );
      case "user-register":
      ...
      case "admin-users-loans":
        return (
          <AdminUserLoanList
            navigate={navigate}
          />
        );
      case "user-loan-analytics":
        return (
          <UserLoanAnalytics
            navigate={navigate}
          />
